...
1. The current phone number validation ensures numbers must be at least 3 digits long and consist of digits (0 to 9). While spaces and dashes are allowed, only 1 space or hyphen is allowed between each digit. This supports common formats like `9143 9032`, `872-233-8554`, and `987223414`. The 3-digit minimum was initially designed to accommodate emergency numbers, but since this is a contact book application meant for storing personal and business contact numbers, this minimum length is insufficient.
   We plan to enhance the phone validation by enforcing a minimum of 7 digits and maximum of 15 digits (excluding formatting characters and area code in parentheses) to match real-world phone numbers. Additionally, we will extend the allowed characters to include international symbols such as `+` and parentheses `( )` to support global phone number formats. The plus sign must only appear at the start, and parentheses must come in matching pairs around area codes. The system will automatically format irregularly spaced numbers into proper groups - for example, `22 22 22 22` would be reformatted to `2222 2222`. This will improve the application's flexibility while preventing erroneous data entry, accommodating formats like `+1 (234) 567-8901`, `+44 20 7123 4567`, and `+65 6789-0123`.
